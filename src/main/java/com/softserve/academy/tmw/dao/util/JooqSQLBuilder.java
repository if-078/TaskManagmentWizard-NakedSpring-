    ...
    public Select buildSql() {
        ...
        if (sto.getTag().length > 0) {
            ...
            selectConditionStep = creator.select(id, name, createdDate,  startDate,endDate, estimateTime,
                    field("user.name").as("user"), field("status.name").as("status"), field("priority.name").as("priority"), parentId).from(table)
            ...
            selectConditionStep = creator.select(id, name, createdDate,  startDate,endDate, estimateTime, field("user.name").as("user")
